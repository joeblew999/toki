...
// TestTimezoneConversion tests timezone-aware time formatting (Issue #18).
func TestTimezoneConversion(t *testing.T) {
	...
	writeFiles(t, dir, map[string]string{
		"main.go": `
			...
			func main() {
				// UTC time: 2025-01-20 15:00:00
				utcTime := time.Date(2025, 1, 20, 15, 0, 0, 0, time.UTC)

				// Test 1: LocaleToTimezone returns correct timezone
				enLoc := tokibundle.LocaleToTimezone(language.MustParse("en"))
				deLoc := tokibundle.LocaleToTimezone(language.MustParse("de"))
				jaLoc := tokibundle.LocaleToTimezone(language.MustParse("ja"))
				fmt.Printf("en timezone: %s\n", enLoc.String())
				fmt.Printf("de timezone: %s\n", deLoc.String())
				fmt.Printf("ja timezone: %s\n", jaLoc.String())

				// Test 2: MatchWithOptions converts time correctly
				enReader, _ := tokibundle.MatchWithOptions(tokibundle.MatchOptions{
					Locale:       language.English,
					TimeLocation: enLoc,
				})
				deReader, _ := tokibundle.MatchWithOptions(tokibundle.MatchOptions{
					Locale:       language.German,
					TimeLocation: deLoc,
				})

				// Print converted times (hour should differ based on timezone)
				enConverted := utcTime.In(enLoc)
				deConverted := utcTime.In(deLoc)
				fmt.Printf("UTC 15:00 -> EN: %d:00\n", enConverted.Hour())
				fmt.Printf("UTC 15:00 -> DE: %d:00\n", deConverted.Hour())

				// Test 3: Reader still works for non-time args
				fmt.Println(enReader.String("just text"))
				fmt.Println(deReader.String("just text"))
			}
		`,
	})
	...
	require.Contains(t, outStr, "en timezone: America/New_York")
	require.Contains(t, outStr, "de timezone: Europe/Berlin")
	require.Contains(t, outStr, "ja timezone: Asia/Tokyo")

	// Verify time conversions (UTC 15:00)
	// New York is UTC-5, so 15-5=10
	require.Contains(t, outStr, "UTC 15:00 -> EN: 10:00")
	// Berlin is UTC+1, so 15+1=16
	require.Contains(t, outStr, "UTC 15:00 -> DE: 16:00")

	// Verify basic string translation still works
	require.Contains(t, outStr, "just text")
}

// TestLocaleToTimezone tests the locale-to-timezone mapping function.
func TestLocaleToTimezone(t *testing.T) {
	dir := t.TempDir()
	initGoMod(t, dir, "tstmod")
	writeFiles(t, dir, map[string]string{
		"main.go": `
			...
			func main() {
				...
				for _, loc := range tests {
					...
					tz := tokibundle.LocaleToTimezone(tag)
					if tz != nil {
						fmt.Printf("%s -> %s\n", loc, tz.String())
					} else {
						fmt.Printf("%s -> nil\n", loc)
	...
	require.Contains(t, outStr, "en -> America/New_York")
	require.Contains(t, outStr, "de -> Europe/Berlin")
	require.Contains(t, outStr, "zh -> Asia/Shanghai")
	require.Contains(t, outStr, "ja -> Asia/Tokyo")
	require.Contains(t, outStr, "th -> Asia/Bangkok")
	require.Contains(t, outStr, "vi -> Asia/Ho_Chi_Minh")

	// Specific country mappings
	require.Contains(t, outStr, "en-US -> America/New_York")
	require.Contains(t, outStr, "en-GB -> Europe/London")
	require.Contains(t, outStr, "de-DE -> Europe/Berlin")
	require.Contains(t, outStr, "de-AT -> Europe/Vienna")
	require.Contains(t, outStr, "zh-CN -> Asia/Shanghai")
	require.Contains(t, outStr, "zh-TW -> Asia/Taipei")
}
